...
const initHeader = () => {
  ...
  const mobileHeader = document.querySelector('.mobile-only-header');

  const toggle = document.querySelector('[data-nav-toggle]');
  const nav = document.querySelector('[data-nav]');

  /*
   * Si no existe ningún encabezado, no hacemos nada.
   */
  if (!header && !mobileHeader) return;


  /* --------------------------------------------------
     CAMBIO DE ENCABEZADO AL HACER SCROLL
     -------------------------------------------------- */
  ...
  const onScroll = () => {
    ...
    /*
     * ENCABEZADO MÓVIL
     * Solo existe en la portada móvil.
     *
     * Al bajar:
     * - cambia a blanco
     * - aparece la sombra
   ...
   * para comprobar la posición inicial.
   */
  onScroll();


  /*
   * Escuchamos el desplazamiento.
   */
...
const initParallax = () => {
  ...
  const update = () => {
    ...
    layers.forEach((layer) => {
      ...
      const speed =
        Number(layer.dataset.parallax) || 0.12;
...
const initLightbox = () => {
  ...
  const open = (trigger) => {
    ...
    index =
      Math.max(
        0,
        visibleTriggers().indexOf(trigger)
      );
    ...
    form.addEventListener(
      ...
      (event) => {
        ...
        if (status) {
          ...
          status.textContent =
            'Gracias por escribir. Te responderemos dentro de las próximas 24 horas hábiles.';
...
const initQuotePrefill = () => {
  ...
  if (
    ...
    message.value =
      `Hola, me interesa cotizar: ${product}. Me gustaría conocer opciones de madera, medidas y plazos.`;
